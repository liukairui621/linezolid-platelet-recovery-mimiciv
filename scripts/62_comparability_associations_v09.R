...
ep<-as.data.frame(fread(file.path(R,'cache/phase7_endpoints.csv')));res<-list();ajerr<-0
for(pop in cfg$populations){
 m<-readRDS(file.path(R,'cache',paste0('phase9_',pop,'.rds')));d<-ep[match(m$data$hadm_id,ep$hadm_id),];stopifnot(!anyNA(d$hadm_id),identical(d$drug,m$data$drug));a<-m$data$A
 prefixes<-c('hospital','hospital_single','physical','physical_single')
 ...
 res[[pop]]<-rbindlist(lapply(seq_len(nrow(point)),function(j){q<-quantile(boot[,j,3],c(.025,.975),na.rm=TRUE);data.table(population=pop,metric=rownames(point)[j],LZD=point[j,1],VAN=point[j,2],RD=point[j,3],lower=unname(q[1]),upper=unname(q[2]),diagnostic_gate=m$report$diagnostic_gate)}))
 for(pr in prefixes)for(arm in 0:1){ii<-a==arm;tt<-d[[paste0(pr,'_day')]][ii];ss<-d[[paste0(pr,'_status')]][ii];w<-m$weights[ii]
 ...
 saveRDS(list(point=point,boot=boot),file.path(R,'cache',paste0('phase9_association_',pop,'.rds')))
}
stopifnot(ajerr<1e-10);z<-rbindlist(res);fwrite(z,file.path(R,'reports/CLINICAL_ASSOCIATIONS_v0.9.csv'))
write_json(list(utc=format(Sys.time(),tz='UTC',usetz=TRUE),AJ_max_error=ajerr,estimates=z),file.path(R,'reports/CLINICAL_ASSOCIATIONS_v0.9.json'),pretty=TRUE,auto_unbox=TRUE,digits=10)
print(z)
