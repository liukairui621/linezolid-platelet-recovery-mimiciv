...
plan=R/'config/analysis_plan_v0.6.1.json'
assert json.loads(plan.read_text())['version']=='0.6.1'
...
c.execute('''CREATE OR REPLACE TABLE phase61_platelet_canonical AS SELECT p.subject_id,p.hadm_id,p.specimen_id,
 p.charttime,p.valuenum,min(p.storetime) storetime,count(*) source_rows FROM target_labs p
 SEMI JOIN phase61_eligible_points e USING(hadm_id)
...
drift=c.execute('''SELECT count(*) FROM phase61_eligible_points e LEFT JOIN LATERAL (
 SELECT charttime,min(valuenum) valuenum,count(DISTINCT valuenum) nvals FROM phase61_platelet_canonical x
...
admin_drift=c.execute('''SELECT count(*) FROM phase61_eligible_points e JOIN LATERAL (
...
co=c.execute('''SELECT e.subject_id,e.hadm_id,e.drug,e.t0,e.admittime,e.dischtime,e.deathtime,
 ...
 a.hospital_expire_flag FROM phase61_eligible_points e JOIN admissions a USING(hadm_id) ORDER BY e.hadm_id''').fetchdf()
assert len(co)==json.loads(plan.read_text())['clinical']['phase6_execution']['cohort_n'] and co.hadm_id.nunique()==len(co)
...
la=c.execute('SELECT hadm_id,charttime,valuenum,storetime FROM phase61_platelet_canonical ORDER BY hadm_id,charttime').fetchdf()
...
out.to_csv(R/'cache/phase61_endpoints.csv',index=False)
def hide(x):
...
for row in summary:row['status']={0:'no first event by14d',1:'confirmed recovery',2:'death before recovery',3:'live discharge before recovery'}[row['status']]
...
(R/'reports/ENDPOINT_CONSTRUCTION_v0.6.1.json').write_text(json.dumps(hide(qc),indent=2))
(R/'cache/internal_reports/ENDPOINT_CONSTRUCTION_v0.6.1_exact.json').write_text(json.dumps(qc,indent=2))
(R/'manifests/endpoints_v0.6.1.json').write_text(json.dumps({'plan_sha256':hashlib.sha256(plan.read_bytes()).hexdigest(),
 'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'endpoint_cache_sha256':hashlib.sha256((R/'cache/phase61_endpoints.csv').read_bytes()).hexdigest()},indent=2))
