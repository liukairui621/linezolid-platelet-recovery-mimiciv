...
planfile<-file.path(R,'config/analysis_plan_v0.6.1.json');plan<-fromJSON(planfile)
stopifnot(plan$version=='0.6.1')
raw<-as.data.frame(fread(file.path(R,'cache/phase4_baseline_only.csv')))
eligible<-fread(file.path(R,'cache/phase61_eligible_ids.csv'))$hadm_id
raw<-raw[raw$hadm_id%in%eligible,,drop=FALSE]
cal<-fread(file.path(R,'cache/phase5_calendar.csv'))
raw$mapped_era<-cal$mapped_era[match(raw$hadm_id,cal$hadm_id)]
stopifnot(nrow(raw)==plan$clinical$phase6_execution$cohort_n,!anyNA(raw$mapped_era),!anyDuplicated(raw$hadm_id))
...
write_json(report,file.path(R,'reports/BIAS_REDUCED_OVERLAP_v0.6.1.json'),pretty=TRUE,auto_unbox=TRUE,digits=10)
fwrite(bal,file.path(R,'reports/COVARIATE_BALANCE_v0.6.1.csv'));fwrite(bs,file.path(R,'cache/internal_reports/phase61_bootstrap_diagnostics.csv'))
saveRDS(list(raw=raw,data=x,fit=fit$fit,weights=fit$w,bootstrap=boot,report=report),file.path(R,'cache/phase61_overlap.rds'))
capture.output(sessionInfo(),file=file.path(R,'reports/R_sessionInfo_v0.6.1.txt'))
